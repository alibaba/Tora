...
class AdaLNMixin(BaseMixin):
    def __init__(
        ...
        self.fuser_list = nn.ModuleList([MGF(128, hidden_size) for _ in range(num_layers)])
    ...
    def layer_forward(
        ...
        **kwargs,
        ...
        if kwargs["video_flow_features"] is not None:
            video_flow_feature = kwargs["video_flow_features"][kwargs["layer_id"]]
            fuser = self.fuser_list[kwargs["layer_id"]]
            H, W = video_flow_feature.shape[-2:]
            T = img_hidden_states.shape[1] // H // W
        ...
        if kwargs["video_flow_features"] is not None:
            h = rearrange(img_attention_input, "B (T H W) C -> (B T) C H W", H=H, W=W)
            h = fuser(h, video_flow_feature, T=T)
            img_attention_input = rearrange(h, "(B T) C H W ->  B (T H W) C", T=T)
...
class DiffusionTransformer(BaseModel):
    ...
    def forward(self, x, timesteps=None, context=None, y=None, **kwargs):
        ...
        if video_flow is not None:
            ...
            video_flow_features = self.traj_extractor(video_flow)
            video_flow_features = torch.stack(video_flow_features)
        else:
            video_flow_features = None
        kwargs["video_flow_features"] = video_flow_features
