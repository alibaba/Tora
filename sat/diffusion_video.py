...
class SATVideoDiffusionEngine(nn.Module):
    ...
    def shared_step(self, batch: Dict) -> Any:
        ...
        batch["video_flow"] = self.encode_first_stage(batch["video_flow"].to(x.dtype), batch)
        batch["video_flow"] = batch["video_flow"].permute(0, 2, 1, 3, 4).contiguous()
    ...
    def sample(
        ...
        **kwargs,
        ...
        video_flow = kwargs.get("video_flow")
        denoiser = lambda input, sigma, c, **addtional_model_inputs: self.denoiser(
            self.model, input, sigma, c, concat_images=concat_images, video_flow=video_flow, **addtional_model_inputs
        )
